...
// Função que mostra as sessões de um filme específico
function showMovieSessions(movieId) {
  const movies = getMovies();
  const movie = movies.find(m => m.id == movieId);
  if (!movie) return console.error("Filme não encontrado:", movieId);

  // Esconde a lista de filmes e mostra o container de sessões
  appContainer.classList.add("hide");
  sessionContainer.classList.remove("hide");

  // Monta dinamicamente o HTML das sessões
  ...
  // Reativa o comportamento dos botões de horário (o seu script)
  if (typeof initializeButtons === "function") {
    initializeButtons();
  }

  // Evento de voltar
  const voltarBtn = sessionContainer.querySelector(".voltar-btn");
  voltarBtn.addEventListener("click", () => {
    appContainer.classList.remove("hide");
    sessionContainer.classList.add("hide");
  });
}
